...
# Initialize OpenAI async client with API key
...
async def create_chatbot_single(prompt, max_tokens=64, max_retries=3):
    """Create a single LLM response using async OpenAI API with gpt-5-nano."""
    # gpt-5-mini needs at least ~32 tokens to output [opinion name]; enforce a safe minimum
...
async def run_experiment_async(max_tokens=64, N_sim=100, N=10, parallel_batch_size=50):
    ...
    print(f"\n{'='*70}")
    print(f"🚀 Starting OpenAI gpt-5-mini Experiment (PARALLEL MODE)")
    print(f"{'='*70}")
    ...
    for idx, row in opinion_pairs.iterrows():
        ...
        folder_path = f'gpt-5-mini/results_single_agent/N={N}'
...
async def simulate_hysteresis_step_with_state_async(
    ...
    """
    Simulate one field point in the hysteresis cycle (async version for OpenAI).
...
async def simulate_hysteresis_cycle_direct_control_async(
    ...
    """
    Simulate hysteresis cycles by varying stubborn agents (async version for OpenAI).
    """
    print(f"\n{'='*60}")
    print(f"Starting Hysteresis Cycle Simulation (OpenAI)")
    print(f"{'='*60}")
    ...
    if save_results:
        ...
        results_dir = Path(model_id) / "results_hysteresis_openai" / f"N={N_regular}"
